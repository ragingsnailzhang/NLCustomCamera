...
-(void)applyVideoEffectsToComposition:(AVMutableVideoComposition *)composition WaterMark:(NSString*)waterMark size:(CGSize)size {
    ...
    CGFloat rate = (CGFloat)size.height/KSCREEN_HEIGHT;
    ...
    UIImage *img = [UIImage getBundleImageWithName:@"hd_pic" className:NSStringFromClass([self class])];
    CGFloat imgLayerWidth = 50*rate;
    CGFloat imgLayerHeight = 50*rate;
    CALayer *imgLayer = [CALayer layer];
    imgLayer.contents = (id)img.CGImage;
    imgLayer.frame = CGRectMake(20*rate, size.height - imgLayerWidth - 25*rate, imgLayerWidth, imgLayerHeight);
